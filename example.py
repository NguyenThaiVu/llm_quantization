...
class ToyLinearModel(torch.nn.Module):
    def __init__(self, m=512, n=1024, k=1024):
        super().__init__()
...
def test_static_quant(target_dtype: torch.dtype, mapping_type: MappingType):
    ...
    torch.cuda.synchronize()
    end_time = time.time()
    print(f"Average inference time before quantization over 100 iterations: {(end_time - start_time)/100*1000:.3f} ms")
    ...
    torch.cuda.synchronize()
    end_time = time.time()
    print(f"Average inference time over {n_iter} iterations: {(end_time - start_time)/n_iter*1000:.3f} ms")    
...
if __name__ == "__main__":
    test_static_quant(torch.uint8, MappingType.SYMMETRIC)
